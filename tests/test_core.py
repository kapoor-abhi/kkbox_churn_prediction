...
def test_behavioral_ratio_features():
    processor = KKBoxFeatureEngineering()
    df = pd.DataFrame({
        'total_transactions': [4],
        'auto_renew_count': [3],
        'recent_30_active_days': [8],
        'previous_30_active_days': [4],
        'recent_30_secs': [800.0],
        'previous_30_secs': [200.0],
    })

    processor.fit(df)
    res = processor.transform(df)

    assert res['auto_renew_ratio'].iloc[0] == pytest.approx(0.75)
    assert res['recent_activity_ratio'].iloc[0] == pytest.approx(2.0)
    assert res['recent_listening_ratio'].iloc[0] == pytest.approx(2.0)
